...
const generateMockData = () => {
  return Array.from({ length: 30 }).map(() => ({
    ...
    SellerDtls: {
      ...
      Stcd: faker.number.int({ min: 1, max: 16 }).toString().padStart(2, '0'),
    ...
    ItemList: Array.from({ length: faker.number.int({ min: 1, max: 5 }) }).map(() => ({
      ...
      Unit: faker.helpers.arrayElement(['CKG', 'CGM', 'CEL']),
      ...
      TaxableAmount: faker.commerce.price({ min: 10, max: 1000, dec: 2 }),
      TaxAmount: faker.commerce.price({ min: 10, max: 1000, dec: 2 }),
      TaxPercent: faker.commerce.price({ min: 1, max: 10, dec: 2 }),
    })),
    ValDtls: {
